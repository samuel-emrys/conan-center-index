...
from conan.tools.files import rmdir, copy, replace_in_file, rename, mkdir, get, apply_conandata_patches, export_conandata_patches
...
class CPythonConan(ConanFile):
    ...
    def generate(self):
        if is_msvc(self):
            ...
            toolchain.configure_args.append(f"--enable-shared={yes_no(self.options.shared)}")
            toolchain.configure_args.append(f"--with-doc-strings={yes_no(self.options.docstrings)}")
            toolchain.configure_args.append(f"--with-pymalloc={yes_no(self.options.pymalloc)}")
            toolchain.configure_args.append(f"--with--system-expat")
            toolchain.configure_args.append(f"--with-system-ffi")
            toolchain.configure_args.append(f"--enable-optimizations={yes_no(self.options.optimizations)}")
            toolchain.configure_args.append(f"--with-lto={yes_no(self.options.lto)}")
            toolchain.configure_args.append(f"--with-pydebug={yes_no(self.settings.build_type == 'Debug')}")

            if self._is_py2:
                toolchain.configure_args.append(f"--enable-unicode={yes_no(self.options.unicode)}")
            if self._is_py3:
                toolchain.configure_args.append(f"--with-system-libmpdec")
                toolchain.configure_args.append(f"--with-openssl={self.dependencies['openssl'].package_folder}")
                toolchain.configure_args.append(f"--enable-loadable-sqlite-extensions={yes_no(not self.options['sqlite3'].omit_load_extension)}")
            if self.settings.compiler == "intel":
                toolchain.configure_args.append(f"--with-icc")
            if self.settings.compiler != "gcc":
                toolchain.configure_args.append(f"--without-gcc")
            if self.settings.options.with_tkinter:
                tcltk_includes = []
                tcltk_libs = []

                # FIXME: collect using some conan util (https://github.com/conan-io/conan/issues/7656)
                for dependency_name in ["tcl", "tk", "zlib"]:
                    dep = self.dependencies[dependency_name]
                    tcltk_includes += ["-I{}".format(include_dir) for include_dir in dep.cpp_info.includedirs]
                    tcltk_libs += ["-l{}".format(lib_dir) for lib_dir in dep.cpp_info.libdirs]
                ...
                toolchain.configure_args.append(f"with-tcltk-includes={' '.join(tcltk_includes)}")
                toolchain.configure_args.append(f"with-tcltk-libs={' '.join(tcltk_libs)}")
            if self.settings.os in ["Linux", "FreeBSD"]:
                # Building _testembed fails due to missing pthread/rt symbols
                toolchain.extra_ldflags.append("-lpthread")
            ...
            #    build = _get_gnu_triplet(str(self.settings.os), str(self.settings.arch), str(self.settings.compiler))
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_curses", False):
            ...
            raise ConanInvalidConfiguration("ncurses is not conan 2 compatible")
        if self.options.get_safe("with_bsddb", False):
            # self.requires("libdb/5.3.28")
            raise ConanInvalidConfiguration("libdb is not conan 2 compatible")
    ...
    def _patch_sources(self):
        ...
        if self.options.get_safe("with_curses", False):
            ...
            replace_in_file(self, os.path.join(self.source_folder, "setup.py"),
                                  "curses_libs = ",
                                  "curses_libs = {} #".format(repr(self.dependencies["ncurses"].cpp_info.libs + self.dependencies["ncurses"].cpp_info.system_libs)))
    ...
    def _msvc_package_copy(self):
        ...
        copy(self, "*", src=os.path.join(self.source_folder, "Include"), dst=os.path.join(self.package_folder, self._msvc_install_subprefix, "include"))
        copy(self, "pyconfig.h", src=os.path.join(self.source_folder, "PC"), dst=os.path.join(self.package_folder, self._msvc_install_subprefix, "include"))
        copy(self, "*.py", src=os.path.join(self.source_folder, "lib"), dst=os.path.join(self.package_folder, self._msvc_install_subprefix, "Lib"))
    ...
    def package_info(self):
        ...
        if is_msvc(self):
            ...
            self.cpp_info.components["python"].includedirs.append(os.path.join("include", f"python{self._version_suffix}{self._abi_suffix}"))
            libdir = "lib"
        if self.options.shared:
        ...
        self.cpp_info.components["_embed_copy"].set_property("pkg_config_name", f"python-{py_version.major}-embed")
        ...
        # TODO: Remove when conan 1.x is deprecated
        ...
        self.conf_info.define("user.python:python", python)
        # TODO: Remove when conan 1.x is deprecated
        ...
        self.conf_info.define("user.python:PYTHONHOME", pythonhome)
        # TODO: Remove when conan 1.x is deprecated
        self.user_info.pythonhome = pythonhome

        pythonhome_required = is_msvc(self) or is_apple_os(self)
        self.conf_info.define("user.python:module_requires_pythonhome", pythonhome_required)
        # TODO: Remove when conan 1.x is deprecated
        ...
        self.conf_info.define("user.python:python_root", python_root)
        # TODO: Remove when conan 1.x is deprecated
